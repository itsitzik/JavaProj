...
public class PlanPanelView extends JPanel implements Observer {
	...
	public PlanPanelView(PlanView planView){
		this.planView = planView;
		planView.add(this,BorderLayout.CENTER);
		this.setBounds(0, 30, planView.getWidth() , 600);
	...
	public void printTable(Table table, Graphics g) {
		...
		int i = 1;
		g.drawString("people: " + table.getPeople(), table.getxPos() * size + padding, table.getyPos() * size + (lnSpace * i++));
		g.drawString("smoke?: " + table.isSmoke(), table.getxPos() * size + padding, table.getyPos() * size + (lnSpace * i++));
		g.drawString("start: " + table.getStartTime().format(DateTimeFormatter.ofPattern("HH:mm")) , table.getxPos() * size + padding, table.getyPos() * size + (lnSpace * i++));
		g.drawString("end: " + table.getEndTime().format(DateTimeFormatter.ofPattern("HH:mm")), table.getxPos() * size + padding, table.getyPos() * size + (lnSpace * i++));
		g.drawString("taken?: " + table.getTaken(), table.getxPos() * size + padding, table.getyPos() * size + (lnSpace * i++));
